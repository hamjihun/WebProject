...
주소로 들어가면 로그인 화면 → 홈 화면(서버 모니터링 / 일정 관리 / 계정 관리 타일) 순서로 열린다.
처음 계정은 **admin / rhksflwk1@** 이며 `data/users.json` 에 저장된다 (수집기를 덮어써도 유지).
계정 관리 화면(관리자만)에서 계정을 만들고 계정마다 볼 수 있는 화면을 체크로 정한다.
에이전트 전송(`/api/metrics`)은 토큰으로만 확인하므로 로그인과 무관하다.
비밀번호를 모두 잊으면 `data/users.json` 을 지우고 수집기를 다시 시작하면 초기화된다.
...
| 탭 | 파일 | 용도 |
|---|---|---|
| 대시보드 | `dashboard.html` | TV·벽면용. OS별 서버 도넛, CPU·메모리 Top 5, 점검 필요(장애·운영 현황), 저장장치 사용률 게이지 Top 5, 디스크 소진 임박 표, 이벤트 현황 표. 위험·오프라인이 있으면 배경이 붉게 점멸. `?tv=1` 이면 탭 숨김 (F11 전체화면 권장) |
| 구성도 | `topology.html` | 네트워크 구성도를 직접 그리는 편집기. "편집" → 장비를 드래그 배치, "연결" 도구로 두 장비를 차례로 클릭해 선 연결, "+ 장비"(인터넷·방화벽·라우터·스위치·무선AP·서버·가상화·NAS·스토리지·DB·PC·노트북·프린터·UPS·CCTV 등 아이콘), "+ 영역"(사각형·둥근 사각형·타원·마름모·육각형·구름), "+ 메모". 요소를 클릭하면 오른쪽 패널에서 이름·아이콘·크기·색, 선 종류(실선·파선·점선)·경로(직선·ㄱ자)·화살표·굵기·라벨, 영역 제목 위치·채우기 등을 바꾼다. 복제·삭제·실행취소(Ctrl+Z)·격자·저장(Ctrl+S). 각 장비 상자 안의 LED 가 정상(초록)·주의(주황)·위험/오프라인(빨강, 깜빡임)을 표시하고, 오프라인이면 연결 선이 빨간 파선. 보기 모드에서 서버를 클릭하면 상세가 뜬다 |
| 서버 현황 | `index.html` | 아래 카드 화면 (기존) |
| 백업 | `backup.html` | 월별 달력 형태로 서버×단계(1차 SQL · 2차 Veeam 작업별 · 3차 USB)의 일자별 백업 여부를 점으로 표시. 초록 정상, 주황 경고, 빨강 실패, 빨간 테두리는 있어야 하는데 없는 날, 주말은 회색. 점을 클릭하면 그 날의 파일·시각·크기·결과 상세. 위에 단계별 오늘 상태와 이번 달 성공률 |
| 통계 · 리포트 | `stats.html` | 기간(오늘·7일·30일·90일·1년·월 선택)과 서버를 골라 평균/최대 CPU·메모리 추이, 일별 경고 건수, 가동률, 서버별 요약, 디스크 증감(단일 서버는 일별 사용량 그래프), 경고 이력을 본다. "엑셀(CSV) 다운로드"는 요약·디스크·추이·경고 이력을 한 파일로, "PDF 저장 / 인쇄"는 브라우저 인쇄 창에서 PDF 로 저장. 시간별 집계(`data/hourly.json`)는 수집기 1.9.0 부터 1년 보관 |
...
- 팝업의 "🔕 이 서버 알림 끄기"로 서버별 알림을 전부 끄거나, "이 서버 알림: CPU/메모리/디스크/오프라인" 체크로 종류별로 끌 수 있습니다 (카드에 🔕 표시). 디스크 알림과 일별 증가량 스냅샷은 설정한 점검 시각(기본 11:30)에 하루 한 번 처리됩니다. 🔔 알림 창에서는 전체 알림, CPU/메모리/디스크/오프라인 알림을 각각 켜고 끕니다.
...
| `POST /api/mute` | 서버별 알림 끄기/켜기 (`{"host":"...","muted":true}`) 또는 종류별 (`{"host":"...","rules":{"mem":false}}`) |
| `GET /api/alerts` | 현재 경고와 최근 알림 이력 (기본 10건, `?n=`) |
| `GET /api/hourly?host=&days=` | 시간별 집계 (cpu/메모리 평균·최대, 오프라인 초). host 생략 시 전체 |
| `GET /api/stats?days=30` (또는 `month=2026-09`, `from=2026-01-01&to=2026-03-31`) | 통계 화면용 집계: 서버별 추이(8일 이하 시간별, 그 이상 일별), CPU·메모리 평균/최대, 가동률(오프라인 초 기준), 디스크 증감, 알림 로그 집계 |
| `GET /api/report.csv?days=30` | 위 집계를 엑셀용 CSV(UTF-8 BOM)로 다운로드 |
| `GET/PUT /api/topology` | 구성도 (노드 위치, 연결, 그룹, 메모) |
