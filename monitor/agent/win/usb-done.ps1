...
#   powershell -NoProfile -ExecutionPolicy Bypass -File "C:\Program Files\IMSMonitoringAgent\usb-done.ps1" %ERRORLEVEL% E:\DBBackup
# 첫 인자: robocopy 종료 코드(0~7 정상, 8 이상 실패). 두 번째 인자(선택): 복사 대상 폴더. 생략하면 자동 감지.
param([int]$Code = 0, [string]$Path = '')
...
$r = @{ time = Iso (Get-Date); code = $Code; ok = ($Code -lt 8); drive = ''; path = $Path; newest_file = $null; newest_time = $null; count = 0; free = 0; total = 0 }
try {
  if (-not $Path) {
...
Log "복사 완료 기록: 코드 $Code ($(if ($r.ok) { '정상' } else { '실패' })), $($r.path), 파일 $($r.count)개, 최신 $($r.newest_time)"
