...
| 위치 | 역할 | 실행 위치 |
|---|---|---|
| `server.js` (v1.10.1) | 수집기. Node.js 내장 http. 데이터 수신, 일별 디스크 스냅샷, 상태 스냅샷 `data/state.json`, 카드 순서, 정적 파일 | IMS 서버 192.168.0.9, 포트 **15138**, 작업 스케줄러 `ServerMonitorCollector` |
...
| `dist/IMS-Monitoring-Agent-Setup.exe` (v1.3.2) | 빌드된 설치 파일. `agent/win/build.sh` (makensis) 로 재빌드 | 각 Windows 서버 |
