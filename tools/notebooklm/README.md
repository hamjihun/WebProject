...
- 가린 항목은 두 개의 문서로 남는다.
  - `_민감정보_목록.md` — **값이 들어 있다. 업로드 금지.** 원본 대조용.
  - `06_계정정보 위치.md` — **값이 없다. 이건 업로드한다.** 어느 파일 어느 시트
    몇 행에 적혀 있는지만 담는다. 엑셀은 시트가 수십 개인 경우가 있어서 시트
    이름까지 적는다(`「(21)」 시트 5행`). 머리말에 "값을 답하지 말고 이 위치를
    안내할 것"이라는 지시가 들어 있어서, 비밀번호를 물으면 원본 위치를 알려준다.
